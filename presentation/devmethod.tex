...
\note{
    ...
    \item Formulering konsekvent efter revidering: nemmere at sammenligne, mere sigende.
...
\begin{frame}
    \frametitle{Udviklingsmetode i multiprojektet}
    \StickyNoteFront{As a $\langle$\emph{type of user}$\rangle$,\\\\I want $\langle$\emph{some goal}$\rangle$\\\\so that $\langle$\emph{some reason}$\rangle$}
    \StickyNoteBack{Conditions of satisfaction} 
    % \StickyNoteBack{\ECFAugie{} \large Conditions of satisfaction} % for sjov font
\end{frame}
\note{
  \begin{itemize}
    \item ...
