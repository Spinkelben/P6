...
We suggest that there continues to be roles distributed among groups. It is very important that groups, which fulfill roles that have a large amount of interaction with other groups, take that responsibility seriously and are available in person during the day. Otherwise that can be a source of frustration for the other groups. Examples of roles with many interactions are \emph{Git}, \emph{Product Owner} and \emph{Server} roles\kimnote{-roles}.
...
\end{description}
\kimnote{FYI: When you list future work, then it will often become a subject at the exam. (You should not change this)}
