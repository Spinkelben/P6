...
\begin{description}
  \item[Automatic Deployment of Scripts] Currently, all scripts in the \code{scriptz} repository used on the server are not automatically moved to their corresponding directories on the server. Whenever a change is made to them in the repository, they have to be manually updated on the server. This is time consuming and error prone, and they should instead be automatically deployed.
