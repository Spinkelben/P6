...
This sprint planning proceeds ordinarily\kimnote{delete this word.} as specified by the development method: We first have a sprint planning in the subproject followed by sprint planning in the group.
...
During the \bd sprint planning we choose to work on four user stories and one technical work item\kimnote{What is this, this has not been introduced. Consider defining it in your method section}. They are labeled with a number in parentheses for reference. In the previous sprint we specified precisely the backlog items used and how to formulate user stories (see \sectionref{sec:processspecs}).
...
\begin{description}
  \item[(1)] As a developer I want libraries to have higher priority than other jobs in the build scheduling in Jenkins so that libraries are not bottlenecked when several people are reliant on them\kimnote{how can they be bottlenecked by people relying on them? What they relying mean in this context?}.
  \begin{itemize}
  ...
  \end{itemize}
  \item[(4)] As a future developer I want to know what is in Jenkins and how it is structured so that it it\kimnote{no double words, this is not rap music! ;)} is easy to start working with \kjimnote{with what?, maybe "it"}
  \begin{itemize}
