...
We do the same for the script that starts and waits for the dummy database inserter app. The script can be seen in \appendixref{app:start_wait_db_inserter_parallel}.

To run the monkey tests in parallel we make a new script, as the Jenkins plugin does not support this feature. We choose not to extend the Jenkins plugin with this feature, as it is faster for us to make a script. The script can be seen in \listingref{lst:run_monkey_parallel}.
