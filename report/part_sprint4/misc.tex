...
We give all libraries priority two by default, and all apps priority four. The \emph{Meta-database} library, however, has priority one, as the developers often wait for this specific library to complete.\todo{Read new explanation of queues}
...
\begin{description}
  ...
  \item[Push Enabled] The script checks if there exists a file called \mono{git_disabled} in the Jenkins home folder. If the file exists, we instantly reject the push. The contents of the file is returned to the developer when they try to push. One should write a message detailing why pushing is disabled in the file. This allows us to temporarily disable pushes to Git if we for example are restarting Jenkins, and let the developers know why it is disabled.
\end{description}
...
We need to document our setup in Jenkins for future developers. We describe the project structure, Jenkins configuration, and the Jenkins installation. The document can be seen in \appendixref{app:jenkins_structure}. \todo{Is this long enough??? Andreas havde de gode formuleringer}