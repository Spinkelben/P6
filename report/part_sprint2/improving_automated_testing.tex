\chapter{Improving Automated Building}
...
\todo{This paragraph above is practically the same as chapter organization below?}

\begin{chapterorganization}
  \item in \sectionref{sec:jenkins_restruct} we explain how we restructure jobs in Jenkins;
  \item in \sectionref{sec:monkey_testing_s2} we describe our work towards making monkey testing work;
  \item in \sectionref{sec:s3_appinstallationtest} we set up a test case that installs all apps on the same device, since this has been an issue;
  \item in \sectionref{sec:s3_linecoverage} enable generation of a code coverage report and set up showing lines of code covered in Jenkins;
