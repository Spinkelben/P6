...
\begin{gradlecode}[caption=Part of our Gradle plugin for updating version code (written in Groovy),label=lst:gradle_versioncode]
...
The APK with the highest build number is the newest. Running the script in \listingref{lst:find_newest_apk} with the application id \code{dk.aau.cs.giraf.launcher} will find the APK in line 4 (with version number \mono{v2.3b21}).\todo{nyt! læs}
...
\begin{lstlisting}[language=bash,showstringspaces=false,caption=Bash script that installs the given APKs to an Android device,label=lst:install_apks_on_android_device]
#!/bin/bash
for a in $@
do
