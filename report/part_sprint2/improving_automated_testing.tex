...
The APK with the highest build number is the newest. The build number is the number after the \code{b} in the file name. Running the script in \listingref{lst:find_newest_apk} with the application id \code{dk.aau.cs.giraf.launcher} will find the APK in line 4 (with version number \mono{v2.3b21}).\todo{nyt! læs}
