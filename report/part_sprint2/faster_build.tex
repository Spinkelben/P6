...
\figureref{fig:launcher_build_times_2} shows three bars. \emph{Old slow} shows the build times before the new dependency system with the emulator working correctly. \emph{New few targets} shows the build times with the build targets \code{check connectedCheck assembleRelease}. When running the same build targets as before, there is an improvement in the build times of 43.6 \%, as the dependencies \emph{Local-db}, \emph{Giraf-component}, and \emph{Oasis-lib} are not built. \emph{Barcode-scanner} is still built, as it is not a library used for any other projects than the launcher. The time build time for \emph{Launcher} has increased. This is because additional work has been done on the Launcher by other groups in parallel with us making the dependency changes.

The complete build time with the added Gradle tasks (\emph{New all targets}) is, as expected, more than when running fewer targets. The time for the Barcode-scanner and Launcher has increased because they run additional tasks. Regardless of the additional targets the total build time is 13.7 \% less than with Git submodules.
...
\begin{tikzpicture}[]
  \begin{axis}[
    xbar stacked,
    scale only axis,
    width=\textwidth-\widthof{New few targets}-0.1cm,
    axis y line*= none, axis x line*= bottom,
    %xmajorgrids = true,
    ytick = data,
    yticklabels = {Old slow, New few targets, New all targets},
    yticklabel style={inner sep=0pt, align=right, xshift=-0.1cm},
    tick align = outside,
    %xtick pos = left,
    bar width=6mm,
