...
\figureref{fig:launcher_build_times_2} shows three bars. \emph{Old slow} shows the build times before the new dependency system with the emulator working correctly. \emph{New limited targets} shows the build times with the build targets \code{check connectedCheck assembleRelease}. When running the same build targets as before, there is an improvement in the build times, as the dependencies \emph{Local-db}, \emph{Giraf-component}, and \emph{Oasis-lib} are not built. \emph{Barcode-scanner} is still built, as it is not a library used for any other projects than the launcher. It can be seen that the build time for \emph{Launcher} has increased dramatically. This is because there is now a UI test that takes a significant amount of time \todo{Redo test without UI test}. This test was not present when we did the initial timing of the entire launcher project.
