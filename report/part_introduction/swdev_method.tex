...
Two work products from Scrum are used, and every group is expected to produce these artifacts \todo{Is an individual group really expected to produce a product backlog? The non-group release backlog?}:
...
\begin{description}
  \item[User Story] These user stories are the functional requirements for the product from a user perspective \parencite{larman2003}. They must be sufficiently small for them to be completed in one sprint \parencite{cohn2009}. \cite{cohn2009} present the following template for a user story: 
