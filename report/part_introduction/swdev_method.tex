...
\begin{description}
  \item[Features] We represent the functional requirements to the product as user stories, as they are lightweight and fit well into the agile principles \parencite{larman2003}. The user story must be sufficiently small for it to be completed in one sprint \parencite{cohn2009}. Cohn \cite{cohn2009} presents the following template for a user story: 
  $$\text{As a $\langle{}$\emph{type of user}$\rangle{}$, I want $\langle{}$\emph{some goal}$\rangle{}$ so that $\langle{}$\emph{some reason}$\rangle{}$}.$$
  This template is used for user stories in the multi-project. If the user story is suggested by developer, the source of the user story should be noted in case there are questions about it. Because bugs also represents behavior the user wants to be changed, they are considered no different from features \parencite{product-backlog2015}.
