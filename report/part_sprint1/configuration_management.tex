...
Configuration management can greatly ease the workflow of the developers and increase their productivity.\todo{Source?} This chapter describes how we plan and execute configuration management. We aim to make it as easy as possible for the developers, and therefore we will automate as much as possible.
...
A configuration item is a piece of software or a combination of hardware and software which is managed as a single entity\parencite{swebok}. Due to the desire of the groups to minimize overhead, there is no formal process which explicitly identifies the items that are to be controlled. The configuration items are identified by the groups of previous years, and by the groups during the project startup. New items might be discovered in the future, and they will be discussed at the status meetings. The currently identified items are:
\begin{itemize}
    \item Every application, and the source code, resources and requirements. 
    \item The supported versions of the Android OS.
    \item The supporting tools, Gradle, and Android studio in particular.
\end{itemize} 
In the following section we will discuss how we aim to manage changes to these items.  

\subsection{Vision for Software Configuration Management}
In this section we describe how we hope to manage the software configurations. 

\begin{description}
  \item[Build Management] The multi-project consist of many applications with inter-dependencies. The source code for the applications are controlled with git\parencite{gitwebsite}. Each application has it own repository. The external dependencies are managed with Gradle which gets the correct version of each external library from Maven repositories. That way, we can decide if we want to update when new versions of our dependencies are released. 
...
Additionally the plugin requires the installation of the apps to test. This means that we need the APKs of all the apps. On the server there is a directory that contains the APKs of all applications. However, many of these APKs are very outdated (some being several years old), and not all of them are signed (so that they can be installed on an Android device). The directory contains many different APKs for each project stores in various different directories. In general it is very hard to know which APKs we must install on an Android device to run monkey testing.

On Jenkins there is a project called \emph{deployment} which builds all other projects, signs (only some) them and move them to the aforementioned directory. We remove this project and make it a part of every project to do this themselves, and ensure there are no redundant APKs present in the APK directory. By making it a part of every project we also ensure that there is always an up-to-date version of all applications whenever they pass a build, complementing our vision for continuous integration. However, this additional work was unplanned, and we do not have time for completing this, and thus the automated monkey testing in this sprint.
