...
To organize these teams Scrum of Scrums \todo{Vi burde nok ikke kalde det Scrum of Scrums, da alle grupper ikke kører Scrum, og derfor er det egentlig ikke Scrum of Scrums vi kører. Det er nærmere Scrum of ``something''. Generelt burde vi ikke kalde mange af tingene Scrum, da det ikke er Scrum.} is utilized. The project period is divided into four sprints. There are a total of three levels: The group level, subproject level, and the multi-project level.
...
\subsection{Meetings}
Each week there is a meeting at the multi-project level. At this meeting the status of the groups in the multi-project is presented, and he development method is evaluated. This ensures that all groups knows the current status of the project, and that the development method can be continuously \todo{Er det den rigtige cont?} improved.
