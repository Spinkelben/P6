...
To organize these teams we use Scrum of Scrums \todo{Vi burde nok ikke kalde det Scrum of Scrums, da alle grupper ikke kører Scrum, og derfor er det egentlig ikke Scrum of Scrums vi kører. Det er nærmere Scrum of ``something''. Generelt burde vi ikke kalde mange af tingene Scrum, da det ikke er Scrum.}. The project period is divided into four sprints. There are a total of three levels: The group level, subproject level, and the multi-project level.
...
On the multi-project level we use a product backlog that contains all the user stories for the entire multi-project. The purpose of the multi-project level is to maintain the product backlog. The user stories in the product backlog are categorized according to each subproject. This enables each subproject to manage their user stories themselves, removing unnecessary management from the multi-project level.
...
\todo{Smid dette ned i sprint planning kapitlet.}
