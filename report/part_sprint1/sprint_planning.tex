...
\todo{Should this chapter have a organization section as well?}

As mentioned in \sectionref{sec:meetings} there are three sprint planning meetings. The first sprint is different, because the process is not fully specified at this point. Sprint planning meetings are held on three levels instead: Multi-project, subproject and group \todo{How is this different from the first sentence in this paragraph?}. Before that can happen, we need to look at the backlog from last year.

\section{User Stories Aggregation}
\todo{Mention that this specifically is something we only did first sprint.}
